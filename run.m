...
p_op = ones(4,1)*.25;%[0.3670 0.2443 0.1461 0.2426];
for i = 1:N_ems
    probs_ems(i,:) = p_op;
end
EMS = [EMS, probs_ems];
...
[K0, w, mu1, mu2, sigma, u, v, new_P] = EMH(Data, K0, w, mu1, mu2, sigma, 50, 0.1, 100);
...
set(hline1,'LineWidth',1.2);
set(hline2,'LineWidth', 1.2);
sgtitle('Histogram of K0 on 50 Experiments')
...
sgtitle('Histogram of w on 50 Experiments')

subplot(2,2,1)
histogram(siglist(:,1), 'FaceColor',[0.4,0.6,0.7])
...
set(hline1,'LineWidth',1.2);
set(hline2,'LineWidth', 1.2);title('Group 4');
sgtitle('Histogram of sigma on 50 Experiments')
...
set(hline1,'LineWidth',1.2);
set(hline2,'LineWidth', 1.2);title('Group 4');
sgtitle('Histogram of mu on 50 Experiments')
...
% OP = datasample(1:N, ceil(N/10),'Replace', false);
% N_op = ceil(N/10);
for rep = 1:length(iters)
perc = iters(rep);
N = size(Geodata,1);
label = ones(N,1)*2;
X1 = datasample(setdiff(1:N,OP),floor(perc*N/10),'Replace', false);
label(X1) = 1;
...
% ensure that converged probabilities recover ground truth
group = zeros(N,1);
for i = 1:N
    if Data(i,5)==1
...
plot([0.1:0.1:1, 2:9], ll1(:,2));
hold on
plot([0.1:0.1:1, 2:9], ll2);
legend('ll(A) proposed model','ll(A) simple model');
ylabel('Log-likelihood of the model');
xlabel('% of size of A over size of B');
hold off

plot([0.1:0.1:1, 2:9], ll1(:,1));
hold on
plot([0.1:0.1:1, 2:9], ll3);
ylabel('Log-likelihood of the model');
xlabel('% of size of A over size of B');
legend('ll(B) proposed model','ll(B) simple model');
hold off


plot([0.1:0.1:1, 2:9], sum(ll1,2));
hold on
plot([0.1:0.1:1, 2:9], ll3+ll2);
ylabel('Log-likelihood of the model');
xlabel('% of size of A over size of B');
legend('proposed model','simple model');
hold off
